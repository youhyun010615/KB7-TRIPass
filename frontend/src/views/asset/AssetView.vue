...
<template>
  <main class="asset-page">
    <div class="shell">
      ...
      <div class="section-title"><h2>연동 계좌</h2><button type="button" @click="router.push({ path: '/profile/financial', query: { step: 2, from: 'asset' } })">＋ 계좌 추가</button></div>
      <section class="accounts">
        <article v-for="account in asset.accounts" :key="account.id" :class="{ primary: account.primary }">
          <button class="account-main" type="button" @click="router.push(`/asset/accounts/${account.id}`)">
          ...
          <button class="delete-account" type="button" :aria-label="`${account.name} 계좌 삭제`" @click="removeAccount(account)">삭제</button>
        </article>
