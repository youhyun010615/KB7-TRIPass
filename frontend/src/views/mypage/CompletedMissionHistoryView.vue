...
function missionTitleParts(title) {
  return String(title || '').split(/(\d+%)/).filter(Boolean).map((text) => ({
    text,
    rate: /^\d+%$/.test(text),
  }))
}
            ...
            <section v-for="week in record.weeks" :key="week.weekNumber" class="week-section">
              ...
              <div class="mission-rows">
                <div v-for="mission in week.missions" :key="mission.id" class="mission-row">
                  ...
                  <strong class="mission-title"><span v-for="(part, index) in missionTitleParts(mission.title)" :key="`${mission.id}-${index}`" :class="{ 'mission-rate': part.rate }">{{ part.text }}</span></strong>
...
.total-card{padding:17px 22px 15px;border-radius:18px}.total-kicker{font-size:14px;letter-spacing:-.01em}.total-amount{margin-top:8px}.total-amount strong{font-size:26px}.total-divider{margin:12px 0 10px}.total-footer{justify-content:flex-start}.month-summary{grid-template-columns:82px 1fr 20px;padding-block:17px}.month-summary>strong{font-size:15px;font-weight:500}.month-summary>span b{color:#10a57e}.month-summary>i{transition:none}.month-card.expanded .month-summary>i{transform:none}.week-header>span{display:flex;align-items:center;gap:7px}.week-header>span em{padding:3px 7px;border:1px solid #0fae96;border-radius:7px;color:#0b9d87;font-size:9px;font-style:normal;font-weight:900}.week-header>span b{color:#0fae96;font-size:12px}.mission-row img{width:17px;height:17px}.total-kicker,.total-amount strong,.month-summary>span em,.week-header>span b,.mission-row>b{font-family:inherit;letter-spacing:-.03em}.mission-rate{color:#2675ea;font-weight:900}
</style>
