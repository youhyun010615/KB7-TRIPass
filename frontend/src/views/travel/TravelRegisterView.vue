  ...
  <main class="register-page" :aria-busy="store.loading">
    ...
    <template v-else-if="step === 3">
      <section class="budget-hero">
        ...
        <div><small>TRIPASS AI RECOMMENDATION</small><h2>여행 예산을 선택해 주세요</h2><p>AI 추천을 참고해 그대로 사용하거나, 내 계획에 맞게 직접 조정할 수 있어요.</p></div>
      </section>
      <section
      ...
      >
        <header>
          <div><span>{{ plan.flag }}</span><strong>{{ plan.name }}</strong><small>{{ dateLabel(plan.startDate) }} ~ {{ dateLabel(plan.endDate) }}</small></div>
        </header>
        ...
        <section class="ai-recommendation">
          ...
          <p v-if="plan.aiReason">{{ plan.aiReason }}</p>
        </section>

        <div class="manual-heading"><div><b>내 여행 예산</b><span>추천 금액을 직접 조정할 수 있어요</span></div><em>직접 입력</em></div>
        <div class="budget-section-title"><b>사전 지출</b><span>저축 목표에서 제외</span></div>
        <div class="budget-grid prepaid-grid">
          <label v-for="category in budgetCategories.filter((item) => item.prepaid)" :key="category.field">
            ...
            <small>AI {{ money(plan.recommendedBudget[category.field]) }}</small>
            <div><input type="number" min="0" max="100000000" :value="plan.budget[category.field]" @input="store.updateBudget(plan.countryId, category.field, $event.target.value)"><em>원</em></div>
          </label>
        </div>
        <div class="subtotal prepaid"><span>사전 지출 합계</span><b>{{ money(countryPrepaidTotal(plan)) }}</b></div>
        <div class="budget-section-title local"><b>현지 여행 자금</b><span>TRIP 월렛 저축 목표</span></div>
