...
export const useTravelScheduleStore = defineStore('travelSchedule', () => {
  ...
  // 예전엔 가장 이른 일정의 날짜를 "오늘"처럼 취급해서(demoToday), 그 일정이 "다가오는 일정" 목록에서
  // 빠지고 "오늘 일정" 카드(클릭해야 펼쳐지는 모달) 안에만 들어가 있어 목록에 누락된 것처럼 보였다.
  // 실제 오늘 날짜를 기준으로 삼아야 지나지 않은 일정이 전부 "다가오는 일정"에 나온다.
  ...
  function toSchedulePayload(payload) {
    return {
      tripCountryId: travel.tripCountryIdByCode[payload.countryCode],
  ...
  async function save(payload) {
    const tripCountryId = travel.tripCountryIdByCode[payload.countryCode]
