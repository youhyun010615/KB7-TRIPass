...
<template>
  <main class="receipt-page">
    ...
    <section class="receipt-list">
      ...
      <button v-for="item in filtered" :key="item.id" type="button" @click="router.push(`/receipt/${item.id}?tripId=${tripId}`)"><span>{{ item.flag }}</span><div><b>{{ item.merchant }}</b><small>{{ item.date }} · {{ item.country }} · {{ item.category }}</small></div><strong>{{ item.currency }} {{ item.amount.toLocaleString() }}</strong><em>›</em></button>
