...
const stepTitle = computed(() => ['여행 계획 등록', '여행 일정 입력', 'AI 여행 예산', '여행 목표 확인'][step.value - 1])
...
<template>
  <main class="register-page" :aria-busy="store.loading">
    ...
    <template v-if="step === 1">
      ...
      <div v-if="store.selectedPlans.length" class="selection-summary">
        <strong>{{ store.selectedPlans.length }}개국을 선택했어요</strong>
        <span>{{ store.selectedPlans.map((plan, index) => `${index + 1}. ${plan.name}`).join(' → ') }}</span>
      </div>
    ...
    <template v-else-if="step === 2">
      ...
      <button class="primary-cta" :disabled="!store.canReviewPlan || store.loading" @click="requestRecommendation">
        {{ store.loading ? 'AI 예산을 계산하고 있어요…' : 'AI 추천 예산 받기' }}
      </button>
    </template>

    <template v-else-if="step === 3">
      <section class="ai-guide result-guide">
        <span>✈</span>
        <div><b>국가·일정 기반 추천이 도착했어요</b><p>항공·숙소는 사전 지출로 분리되고, 나머지 네 항목만 저축 목표에 포함돼요.</p></div>
      </section>
      <section
      ...
      >
        ...
        <p v-if="plan.aiReason" class="ai-reason">{{ plan.aiReason }}</p>
