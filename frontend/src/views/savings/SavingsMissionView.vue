...
<template>
  <main class="mission-page tab-scroll-surface" data-tab-scroll>
    ...
    <section v-else-if="isTraveling" class="state-card travel-mission-lock">
      <div class="travel-status-motion" aria-hidden="true">
        <i class="travel-pulse pulse-one"></i>
        <i class="travel-pulse pulse-two"></i>
        <span class="travel-location-marker"><b></b></span>
        <small class="travel-route-dot dot-one"></small>
        <small class="travel-route-dot dot-two"></small>
        <small class="travel-route-dot dot-three"></small>
      </div>
      <h2>여행 중에는 새로운 미션이<br>제공되지 않아요</h2>
      <p>지금까지 완료한 미션은 마이페이지 &gt; 여행 관리에서 언제든 확인할 수 있어요.</p>
      <button type="button" @click="router.push(`/mypage/travel/${travelStore.lifecycle?.tripId}`)">완료한 미션 보기</button>
    </section>
...
.travel-status-motion{position:relative;width:90px;height:82px;margin:0 auto 4px}.travel-location-marker{position:absolute;top:13px;left:50%;z-index:3;width:38px;height:38px;border-radius:50% 50% 50% 6px;background:linear-gradient(145deg,#2662ea,#173f8d);box-shadow:0 9px 18px rgba(23,63,141,.24);transform:translateX(-50%) rotate(-45deg);animation:travel-marker-float 2s ease-in-out infinite}.travel-location-marker b{position:absolute;top:11px;left:11px;width:16px;height:16px;border:4px solid #fff;border-radius:50%;transform:rotate(45deg)}.travel-pulse{position:absolute;top:44px;left:50%;width:52px;height:18px;border:1.5px solid rgba(47,112,242,.36);border-radius:50%;transform:translateX(-50%);animation:travel-location-pulse 2s ease-out infinite}.travel-pulse.pulse-two{animation-delay:1s}.travel-route-dot{position:absolute;bottom:5px;width:5px;height:5px;border-radius:50%;background:#9db7e8;animation:travel-route-flow 1.8s ease-in-out infinite}.travel-route-dot.dot-one{left:24px}.travel-route-dot.dot-two{left:43px;animation-delay:.25s}.travel-route-dot.dot-three{left:62px;animation-delay:.5s}@keyframes travel-marker-float{0%,100%{transform:translateX(-50%) translateY(0) rotate(-45deg)}50%{transform:translateX(-50%) translateY(-6px) rotate(-45deg)}}@keyframes travel-location-pulse{0%{opacity:.8;transform:translateX(-50%) scale(.55)}100%{opacity:0;transform:translateX(-50%) scale(1.35)}}@keyframes travel-route-flow{0%,100%{opacity:.28;transform:scale(.7)}50%{opacity:1;transform:scale(1.25)}}
@media(prefers-reduced-motion:reduce){.travel-location-marker,.travel-pulse,.travel-route-dot{animation:none}}
...
.mission-savings-card>p{font-size:13px;font-weight:800;letter-spacing:.1em;color:#ffd466}
