...
const unreadCount = computed(() => store.schedules.filter(item => !item.notificationRead && item.alert !== '알림 없음').length)
</script>
...
<template>
  <main class="schedule-page">
    ...
    <section v-if="todaySchedules.length" class="today-ticket">
      ...
      <button v-for="item in todaySchedules" :key="item.id" type="button" @click="router.push(`/schedule/${item.id}`)">
        <time>{{ item.time }}</time><span><b>{{ country(item.countryCode).flag }} {{ item.title }}</b><small>{{ item.currency }} {{ item.amount.toLocaleString() }} · {{ item.paymentStatus === 'prepaid' ? '사전결제 완료' : '현장결제 필요' }}</small></span><em @click.stop="store.toggleComplete(item.id)">{{ item.completed ? '완료' : '일정 완료' }}</em>
      </button>
