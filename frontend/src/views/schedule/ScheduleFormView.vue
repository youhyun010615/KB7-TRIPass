...
const form = reactive(original ? { ...original } : {
  title:'', date:store.travelStart, time:'10:00', currency:'EUR', amount:0, paymentStatus:'prepaid', place:'', alert:'1시간 전', memo:'',
})
...
function submit() {
  ...
  const payload = { ...form, amount:Number(form.amount), title:form.title.trim(), place:form.place.trim(), memo:form.memo.trim() }
...
<template>
  <main class="form-page">
    ...
    <section class="form-card">
      ...
      <label><span>🔔 알림</span><select v-model="form.alert"><option>알림 없음</option><option>30분 전</option><option>1시간 전</option><option>2시간 전</option><option>1일 전</option></select></label>
