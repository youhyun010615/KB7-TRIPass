...
import moneyTripIcon from '@/assets/icons/money_trip.svg'
import preMoneyIcon from '@/assets/icons/pre_money.svg'
      ...
      >
        ...
        <section class="budget-recommendation-panel">
          ...
          <section v-if="budgetBasisOpen" class="budget-basis-detail">
            ...
            <footer><b>2026.08 조사 기준.</b><span>실제 예약 시점과 여행 방식에 따라 달라질 수 있어요.</span></footer>
          </section>
        </section>
...
.budget-basis-ai{flex:none;width:20px;height:20px;color:#2469e8;background:transparent;border:1.5px solid #4384f1;animation:none}.budget-basis-ai::after{display:none}.budget-basis-ai b{font-size:6px;animation:none}
...
/* iOS PWA에서는 상태 표시줄까지 앱 배경이 이어지고 홈 인디케이터와 CTA가 겹치지 않게 한다. */
.onboarding-register{
  min-height:100vh;
  min-height:100dvh;
  padding-bottom:calc(104px + env(safe-area-inset-bottom));
}
.onboarding-register .page-header{
  height:calc(62px + env(safe-area-inset-top));
  padding-top:env(safe-area-inset-top);
}
.onboarding-register .register-content{
  /* 종료 뒤 transform을 남기면 내부 fixed CTA가 카드 기준으로 배치된다. */
