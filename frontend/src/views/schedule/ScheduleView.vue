...
const todaySchedules = computed(() => store.sortedSchedules.filter(item => item.date === store.todayFor(item.timeZone)))
const upcomingSchedules = computed(() => store.sortedSchedules.filter(item => item.date > store.todayFor(item.timeZone)))
...
<template>
  <main class="schedule-page">
    ...
    <section v-if="todaySchedules.length" class="today-ticket" role="button" tabindex="0" @click="todayModalOpen = true" @keydown.enter="todayModalOpen = true">
      <div class="ticket-head"><b>오늘 일정</b><small>{{ store.today.replaceAll('-', '.') }}</small></div>
      <div class="cut"><i/><span/><i/></div>
      <p>오늘 일정이 {{ todaySchedules.length }}건 있어요.</p>
      <div class="today-preview-list"><ScheduleCard v-for="item in todaySchedules" :key="item.id" :schedule="item" compact @detail="openDetail" /></div>
    ...
    <section class="upcoming-card">
      <div class="section-title"><h2>다가오는 여행 일정</h2><span>총 {{ upcomingSchedules.length }}건</span></div>
