...
const autoChargeSummary = computed(() => wallet.autoCharge.enabled
  ? `매월 ${wallet.autoCharge.day}일 ${money(wallet.autoCharge.amount)}`
  : '설정하기')
        ...
        <section class="wallet-sheet" :class="{ 'charge-sheet': transferMode === 'charge', 'withdraw-sheet': transferMode === 'withdraw' }">
          <template v-if="transferMode === 'charge'">
            ...
            <div v-if="showMonthlySavingGoalInSheet" class="charge-goal-card">
              <div class="charge-goal-row">
                <span>이번달 목표 금액</span><b>{{ money(monthlySavingGoalTarget) }}</b>
              </div>
              <div class="charge-goal-row">
                <span>남은 저축 금액</span><b class="charge-goal-remaining">{{ money(monthlySavingGoalRemaining) }}</b>
              </div>
            </div>
            ...
            <button type="button" class="auto-charge-row" @click="showTransfer = false; openAutoChargeSetting()">
              <span>자동충전</span>
              <span class="auto-charge-row-value">{{ autoChargeSummary }}<ChevronRight :size="14" /></span>
            </button>
          </template>
...
.auto-charge-row>span:first-child{color:#2f70e9;font-size:14px;font-weight:700}
.auto-charge-row-value{
  display:flex;
  align-items:center;
  gap:2px;
  color:#5f6775;
...
.charge-goal-row>span{color:#5f6775;font-weight:600}
.charge-goal-row>b{color:#1c2433;font-weight:700}
.charge-goal-remaining{color:#173f8d !important}
