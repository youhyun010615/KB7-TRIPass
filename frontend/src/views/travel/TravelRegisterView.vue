...
<template>
  <main
    ...
    :class="{
      ...
      'edit-register': isEditMode,
      'schedule-register-step': step === 2,
  ...
  >
    ...
    <template v-if="step === 1">
      <section v-if="!isEditMode" class="guide-card">
        <b>어디로 떠나시나요?</b>
        ...
        <section class="budget-recommendation-panel">
          ...
          <section v-if="budgetBasisOpen" class="budget-basis-detail">
            ...
            <section class="basis-group basis-prepaid">
              ...
              <article>
                <div class="basis-row"><strong><span class="basis-category-icon">✈</span>항공</strong><b>{{ money(activeBudgetPlan.recommendedBudget.airfareAmount) }}</b></div>
              ...
              <article>
                <div class="basis-row"><strong><span class="basis-category-icon">▦</span>숙박</strong><b>{{ money(recommendedNightlyAmount(activeBudgetPlan)) }} × {{ recommendationStayNights(activeBudgetPlan) }}박 = {{ money(activeBudgetPlan.recommendedBudget.lodgingAmount) }}</b></div>
            ...
            <section class="basis-group basis-local">
              ...
              <article>
                <div class="basis-row"><strong><span class="basis-category-icon">🍴</span>식비</strong><b>{{ money(recommendedDailyAmount(activeBudgetPlan, 'foodAmount')) }} / 일</b></div>
                <em><span aria-label="웹사이트 출처">◎</span>Budget Your Trip · 실제 여행자 식비 지출</em>
                <p>여행자가 기록한 식사·음료 지출 비중을 대표 도시의 전체 일평균과 비교해 과대값을 보정했어요.</p>
              </article>
              <article>
                <div class="basis-row"><strong><span class="basis-category-icon">▣</span>관광</strong><b>{{ money(recommendedDailyAmount(activeBudgetPlan, 'activityAmount')) }} / 일</b></div>
                <em><span aria-label="웹사이트 출처">◎</span>Budget Your Trip · 관광·엔터테인먼트 지출</em>
                <p>입장권·투어·액티비티에 실제 사용한 일평균을 비교해 서비스 기준 단가로 보정했어요.</p>
              </article>
              <article>
                <div class="basis-row"><strong><span class="basis-category-icon">▤</span>교통</strong><b>{{ money(recommendedDailyAmount(activeBudgetPlan, 'transportAmount')) }} / 일</b></div>
              ...
              <article>
                <div class="basis-row"><strong><span class="basis-category-icon">•••</span>기타</strong><b>{{ money(recommendedDailyAmount(activeBudgetPlan, 'otherAmount')) }} / 일</b></div>
    ...
    <template v-else>
      ...
      <template v-else>
        <section class="wallet-plan-summary">
        ...
        <section v-if="walletDisplayAmount <= 0" class="wallet-zero-notice">
          <i><img :src="alertFilledIcon" alt=""></i>
          <div><b>이번 여행 월렛은 0원에서 시작해요</b><p>지금 월렛에 남은 잔액이 없어 여행 자금은 등록 후 새로 모으게 돼요.</p></div>
        </section>

        <section v-else class="wallet-choice-section">
          <h2>월렛 자금 포함 여부</h2>
          <button type="button" class="wallet-choice-card" :class="{ selected: walletChoice === 'include' }" @click="selectWalletChoice('include')">
            <i />
            <div class="wallet-choice-copy">
              <div class="wallet-choice-title"><b>이번 여행에 포함하기</b><strong>+{{ money(walletDisplayAmount) }}</strong></div>
              <p>월렛 잔액이 여행 저축액으로 이어져요. 월별 저축 금액은 {{ money(monthlySavingWithWallet) }}이 돼요.</p>
            </div>
          </button>
          <button type="button" class="wallet-choice-card" :class="{ selected: walletChoice === 'transfer' }" @click="selectWalletChoice('transfer')">
            <i />
            <div class="wallet-choice-copy"><b>포함하지 않고 계좌로 송금하기</b><p>남은 {{ money(walletDisplayAmount) }}을 연결 계좌로 보내고, 월렛은 0원부터 시작해요.</p></div>
          </button>
        </section>

        <section v-if="walletChoice === 'transfer'" class="wallet-transfer-flow">
...
<style scoped>
.goal-total-summary{display:flex;min-height:124px;flex-direction:column;align-items:center;justify-content:center;padding:18px 14px;border-radius:16px;background:#f5f7fb;text-align:center}.goal-total-summary p{color:#98a3b5;font-size:9px;font-weight:700}.goal-total-summary>strong{margin-top:13px;color:#173b86;font-size:10px;font-weight:900;letter-spacing:.04em}.goal-total-summary>strong span{display:inline-block;margin-right:5px;font-size:9px;transform:rotate(-10deg)}.goal-total-summary>b{margin-top:13px;color:#0b2a6b;font-family:'Space Mono',ui-monospace,SFMono-Regular,Menlo,monospace;font-size:29px;line-height:1;font-weight:900;letter-spacing:-.055em}
...
.edit-register.schedule-register-step .register-content{padding-top:10px}.edit-register.schedule-register-step .plan-guide{align-items:flex-start;margin-top:0;padding:16px;border:1px solid #c9dcf6;background:#eef5ff;box-shadow:none}.edit-register.schedule-register-step .plan-guide div{gap:5px}.edit-register.schedule-register-step .plan-guide b{font-size:14px}.edit-register.schedule-register-step .plan-guide span{color:#607798;font-size:10px}.edit-register.schedule-register-step .plan-guide button{align-self:flex-start;margin:-2px 0 0 auto;padding:2px 0;color:#2469e8;font-size:10px;font-weight:900}
...
.edit-register.budget-summary-step .budget-fixed-summary{width:calc(100% + 32px);margin-right:-16px;margin-left:-16px}
...
.wallet-transfer-flow{color:#10192b}.wallet-transfer-flow>small{color:#c58f00;font-family:ui-monospace,monospace;font-size:9px;font-weight:900;letter-spacing:.13em}.wallet-transfer-flow h2{margin-top:13px;font-size:19px}.wallet-transfer-flow>p{margin-top:6px;color:#70809a;font-size:9px}.wallet-transfer-summary{margin-top:18px;padding:18px;border:1px solid #e0e7f0;border-radius:18px;color:#10192b;background:#fff}.wallet-transfer-summary>span{color:#8a97ab;font-size:10px;font-weight:800}.wallet-transfer-summary>strong{display:block;margin-top:9px;padding-bottom:15px;border-bottom:1px solid #edf0f5;font-family:'Space Mono',ui-monospace,monospace;font-size:27px}.wallet-transfer-summary>div{display:flex;justify-content:space-between;margin-top:10px;color:#8a97ab;font-size:9px}.wallet-transfer-summary>div b{color:#173b86;font-size:11px}.wallet-transfer-flow h3{margin-top:17px;font-size:11px}.wallet-inline-account-list{display:grid;gap:8px;margin-top:9px}.wallet-inline-account-list button{display:flex;width:100%;align-items:center;gap:10px;padding:13px;border:1px solid #e0e7f0;border-radius:14px;color:#10192b;background:#f5f7fb;text-align:left}.wallet-inline-account-list button.selected{border:2px solid #ffd45e;color:#10192b;background:#fff}.wallet-inline-account-list button div{flex:1}.wallet-inline-account-list b{font-size:10px}.wallet-inline-account-list small{display:block;margin-top:4px;color:#8a97ab;font-size:8px}.wallet-inline-account-list em{padding:5px 7px;border-radius:999px;color:#173b86;background:#fff0bd;font-size:8px;font-style:normal;font-weight:900}.wallet-transfer-flow .account-connect-link{color:#2469e8}
