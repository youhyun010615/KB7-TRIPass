...
-- ============================================================================
SET @cleanup_uid = (SELECT id FROM users WHERE login_provider = 'LOCAL' AND login_id = 'yuhyun4');
...
SET @user_id = (SELECT id FROM users WHERE login_provider = 'LOCAL' AND login_id = 'yuhyun4');
...
VALUES (@user_id, 'MOCK-CONNECTED-YUHYUN4', 'ACTIVE', '2026-08-22 14:30:00', '2026-08-22 14:30:00')
ON DUPLICATE KEY UPDATE connection_status = 'ACTIVE', last_synced_at = NOW();

SET @codef_conn_id = (SELECT id FROM codef_connections WHERE user_id = @user_id AND connected_id = 'MOCK-CONNECTED-YUHYUN4');
...
VALUES
    (@user_id, @codef_conn_id, 'KB nori 체크카드', '5412-****-****-9907', 'CHECK',
     '0301', '496501-01-110303', @account_id, '2026-08-22 14:30:00', '2026-08-22 14:30:00'),
    (@user_id, @codef_conn_id, 'KB 트래블러스 체크카드', '5412-****-****-9908', 'CHECK',
     '0301', '496501-01-110303', @account_id, '2026-08-22 14:30:00', '2026-08-22 14:30:00')
...
VALUES (@user_id, @travelcard_master_id, '트래블러스 체크카드', 'KB국민카드', '5412-****-****-9908',
        'CODEF', 'BLUE', 'ACTIVE',
        CONCAT('CODEF:0301:5412-****-****-9908'), FALSE, '2026-08-22 14:30:00', '2026-08-22 14:30:00')
ON DUPLICATE KEY UPDATE
...
VALUES
    ...
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'MANUAL',
     ...
     'DEMO-SEED-08-Y4', '8월 여행 저축 (D-day 첫달)', '2026-08-25 12:00:00'),

    -- 9월 저축
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     658000, 193000, 851000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-09-Y4', '9월 여행 저축', '2026-09-25 09:00:00'),

    -- 10월 저축 (추가 +150,000)
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     658000, 851000, 1509000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-10A-Y4', '10월 여행 저축', '2026-10-25 09:00:00'),
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'MANUAL',
     150000, 1509000, 1659000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-10B-Y4', '10월 추가 저축', '2026-10-28 20:00:00'),

    -- 11월 저축
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     658000, 1659000, 2317000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-11-Y4', '11월 여행 저축', '2026-11-25 09:00:00'),

    -- 12월 저축 (목표 미달 -150,000)
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     508000, 2317000, 2825000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-12-Y4', '12월 여행 저축 (목표 미달)', '2026-12-25 09:00:00'),

    -- 1월 저축 (보충 +200,000)
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     658000, 2825000, 3483000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-01A-Y4', '1월 여행 저축', '2027-01-25 09:00:00'),
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'MANUAL',
     200000, 3483000, 3683000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-01B-Y4', '1월 보충 저축 (12월 미달분)', '2027-01-28 20:00:00'),

    -- 2월 저축 (추가 +150,000)
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     658000, 3683000, 4341000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-02A-Y4', '2월 여행 저축', '2027-02-25 09:00:00'),
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'MANUAL',
     150000, 4341000, 4491000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-02B-Y4', '2월 추가 저축', '2027-02-27 20:00:00'),

    -- 3월 저축 (추가 +100,000)
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     658000, 4491000, 5149000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-03A-Y4', '3월 여행 저축', '2027-03-25 09:00:00'),
    (@wallet_id, @trip_id, 'IN', 'CHARGE', 'MANUAL',
     100000, 5149000, 5249000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-03B-Y4', '3월 추가 저축', '2027-03-28 20:00:00');
