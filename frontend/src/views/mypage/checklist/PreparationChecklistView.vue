...
const stageMeta = {
  ...
  d7: { label: 'D-7', title: '이번 단계 항목' },
  d1: { label: 'D-1', title: '출국 직전 항목' },
};
const items = computed(() => store.rolledItems(tripId.value, stage.value));
const ownItems = computed(
  () => store.getTrip(tripId.value).preparation[stage.value],
);
const regularItems = computed(() =>
  items.value.filter((item) =>
    ownItems.value.some((own) => own.id === item.id),
  ),
);
const rolled = computed(() =>
  items.value.filter(
    (item) => !ownItems.value.some((own) => own.id === item.id),
  ),
);
const progress = computed(() => store.progress(items.value));
function submit() {
  if (store.add(tripId.value, 'preparation', stage.value, newItem.value)) {
    newItem.value = '';
    adding.value = false;
  }
}
...
<template>
  <main class="page">
    ...
    <section class="pass">
      ...
      <div>
        <h2>{{ stageMeta[stage].label }} 체크리스트</h2>
    ...
    <section class="items">
      <button
        ...
        @click="store.toggle(tripId, 'preparation', item.id)"
      >
        <span>{{ item.done ? '✓' : '' }}</span>
        <div>
          <b>{{ item.text }}</b
    ...
    <template v-if="rolled.length"
      ...
      <section class="items rolled">
        ...
        >
          ...
          <div>
            <b>{{ item.text }}</b
            ><small>{{ item.origin }} 미완료 항목</small>
          </div>
          <em>이월</em>
        </button>
      </section></template
    ...
    <section v-if="adding" class="add-box">
      <textarea
        v-model="newItem"
        maxlength="60"
        placeholder="추가할 여행 준비 항목을 입력하세요"
      />
      <div>
