...
watch(() => route.name, (routeName) => {
  ...
  if (routeName === 'TravelRegister') step.value = 1
...
function goToAccountOnboarding() {
  router.replace({ name: 'FinancialProfile', query: { onboarding: '1' } })
}
</script>
...
<template>
  <main class="register-page" :aria-busy="store.loading">
    <header class="page-header">
      ...
      <button v-if="isOnboarding" type="button" class="onboarding-skip" @click="goToAccountOnboarding">다음에 할게요</button>
      <span v-else />
    </header>
