...
import { Bell, ChevronRight, CreditCard } from '@lucide/vue'
...
async function openTransfer(mode) {
  ...
  selectedAccount.value = accounts.value.find(account => account.isPrimary)?.accountId ?? accounts.value[0]?.accountId ?? null
...
async function submitTransfer() {
  ...
  try {
    const ok = transferMode.value === 'charge'
      ? await wallet.deposit(amount.value, selectedAccount.value)
      : await wallet.withdraw(amount.value, selectedAccount.value)

    if (!ok) {
      showNotice(transferMode.value === 'charge' ? '채울 금액을 입력해 주세요.' : '월렛 잔액 안에서 뺄 금액을 입력해 주세요.')
      return
    }

    showNotice(transferMode.value === 'charge'
      ? `${money(amount.value)}을 월렛에 채웠어요.`
      : `${money(amount.value)}을 연결 계좌로 뺐어요.`)
...
<template>
  <main class="wallet-page">
    ...
    <header class="wallet-header">
      ...
      <button type="button" class="alarm-button" aria-label="자동 채우기 기록" @click="router.push('/wallet/notifications')">
        <Bell :size="22" />
      </button>
    </header>
    ...
    <Transition name="sheet">
      <div v-if="showTransfer" class="wallet-sheet-backdrop" @click.self="showTransfer = false">
        <section class="wallet-sheet">
          ...
          <p>{{ transferMode === 'charge' ? '연결된 계좌에서 월렛으로 여행 자금을 옮겨요.' : '월렛 잔액을 연결 계좌로 돌려보내요.' }}</p>
          <label>
            연결 계좌 선택
