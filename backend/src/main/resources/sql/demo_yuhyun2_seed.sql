...
-- ============================================================================
SET @cleanup_uid = (SELECT id FROM users WHERE login_provider = 'LOCAL' AND login_id = 'yuhyun2');
...
SET @user_id = (SELECT id FROM users WHERE login_provider = 'LOCAL' AND login_id = 'yuhyun2');
...
VALUES (@user_id, 'MOCK-CONNECTED-YUHYUN2', 'ACTIVE', '2026-08-22 14:30:00', '2026-08-22 14:30:00')
ON DUPLICATE KEY UPDATE connection_status = 'ACTIVE', last_synced_at = NOW();

SET @codef_conn_id = (SELECT id FROM codef_connections WHERE user_id = @user_id AND connected_id = 'MOCK-CONNECTED-YUHYUN2');
...
VALUES (@user_id, @codef_conn_id, '0004', 'KB국민은행 종합통장', '496501-01-110302',
...
SET @account_id = (SELECT id FROM accounts WHERE user_id = @user_id AND account_number = '496501-01-110302');
...
VALUES
    (@user_id, @codef_conn_id, 'KB nori 체크카드', '5412-****-****-9921', 'CHECK',
     '0301', '496501-01-110302', @account_id, '2026-08-22 14:30:00', '2026-08-22 14:30:00'),
    (@user_id, @codef_conn_id, 'KB 트래블러스 체크카드', '5412-****-****-9922', 'CHECK',
     '0301', '496501-01-110302', @account_id, '2026-08-22 14:30:00', '2026-08-22 14:30:00')
...
VALUES (@user_id, @travelcard_master_id, '트래블러스 체크카드', 'KB국민카드', '5412-****-****-9922',
        'CODEF', 'BLUE', 'ACTIVE',
        CONCAT('CODEF:0301:5412-****-****-9922'), FALSE, '2026-08-22 14:30:00', '2026-08-22 14:30:00')
ON DUPLICATE KEY UPDATE
...
SET @utc_id = (SELECT id FROM user_travel_cards WHERE user_id = @user_id AND external_card_key = 'CODEF:0301:5412-****-****-9922' LIMIT 1);
...
VALUES
    (@wallet_id, @utc_id, @travelcard_master_id, '트래블러스 체크카드', 'KB국민카드', '5412-****-****-9922', 'LINKED');
...
VALUES
    ...
    (@wallet_id, 'IN', 'CHARGE', 'MANUAL',
     ...
     'DEMO-SEED-Y2-08', '8월 첫 저축', '2026-08-26 12:00:00'),

    -- 2. 9/05 자동 저축
    (@wallet_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     420000, 50000, 470000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-Y2-09', '9월 자동 저축 (주)핀테크솔루션', '2026-09-05 09:00:00'),

    -- 3. 10/05 자동 저축
    (@wallet_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     420000, 470000, 890000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-Y2-10A', '10월 자동 저축 (주)핀테크솔루션', '2026-10-05 09:00:00'),

    -- 4. 10/08 수동 추가 저축
    (@wallet_id, 'IN', 'CHARGE', 'MANUAL',
     80000, 890000, 970000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-Y2-10B', '10월 추가 저축', '2026-10-08 20:00:00'),

    -- 5. 10/10 환전 OUT (EUR 300)
    (@wallet_id, 'OUT', 'CARD_TOPUP', NULL,
     480000, 970000, 490000,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-01', 'EUR 환전 300.00', '2026-10-10 14:00:00'),

    -- 6. 11/05 자동 저축
    (@wallet_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     420000, 490000, 910000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-Y2-11', '11월 자동 저축 (주)핀테크솔루션', '2026-11-05 09:00:00'),

    -- 7. 12/05 자동 저축
    (@wallet_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     320000, 910000, 1230000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-Y2-12', '12월 자동 저축 (주)핀테크솔루션', '2026-12-05 09:00:00'),

    -- 8. 12/20 환전 OUT (CHF 200)
    (@wallet_id, 'OUT', 'CARD_TOPUP', NULL,
     340000, 1230000, 890000,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-02', 'CHF 환전 200.00', '2026-12-20 14:00:00'),

    -- 9. 1/05 자동 저축
    (@wallet_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     420000, 890000, 1310000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-Y2-01A', '1월 자동 저축 (주)핀테크솔루션', '2027-01-05 09:00:00'),

    -- 10. 1/08 수동 추가 저축
    (@wallet_id, 'IN', 'CHARGE', 'MANUAL',
     100000, 1310000, 1410000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-Y2-01B', '1월 추가 저축', '2027-01-08 20:00:00'),

    -- 11. 1/20 환전 OUT (EUR 200)
    (@wallet_id, 'OUT', 'CARD_TOPUP', NULL,
     320000, 1410000, 1090000,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-03', 'EUR 환전 200.00', '2027-01-20 14:00:00'),

    -- 12. 2/05 자동 저축
    (@wallet_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     420000, 1090000, 1510000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-Y2-02', '2월 자동 저축 (주)핀테크솔루션', '2027-02-05 09:00:00'),

    -- 13. 2/20 환전 OUT (CHF 300)
    (@wallet_id, 'OUT', 'CARD_TOPUP', NULL,
     510000, 1510000, 1000000,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-04', 'CHF 환전 300.00', '2027-02-20 14:00:00'),

    -- 14. 3/05 자동 저축
    (@wallet_id, 'IN', 'CHARGE', 'AUTO_SAVING',
     420000, 1000000, 1420000,
     'ACCOUNT', @account_id, 'WALLET', @wallet_id,
     'DEMO-SEED-Y2-03', '3월 자동 저축 (주)핀테크솔루션', '2027-03-05 09:00:00'),

    -- 15. 3/20 환전 OUT (EUR 200)
    (@wallet_id, 'OUT', 'CARD_TOPUP', NULL,
     320000, 1420000, 1100000,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-05', 'EUR 환전 200.00', '2027-03-20 14:00:00'),

    -- 16. 3/20 환전 OUT (CHF 400)
    (@wallet_id, 'OUT', 'CARD_TOPUP', NULL,
     680000, 1100000, 420000,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-06', 'CHF 환전 400.00', '2027-03-20 14:30:00');

-- ledger ID 참조용
SET @wl_fx01 = (SELECT id FROM wallet_ledger WHERE wallet_id = @wallet_id AND idempotency_key = 'DEMO-SEED-Y2-FX-01');
SET @wl_fx02 = (SELECT id FROM wallet_ledger WHERE wallet_id = @wallet_id AND idempotency_key = 'DEMO-SEED-Y2-FX-02');
SET @wl_fx03 = (SELECT id FROM wallet_ledger WHERE wallet_id = @wallet_id AND idempotency_key = 'DEMO-SEED-Y2-FX-03');
SET @wl_fx04 = (SELECT id FROM wallet_ledger WHERE wallet_id = @wallet_id AND idempotency_key = 'DEMO-SEED-Y2-FX-04');
SET @wl_fx05 = (SELECT id FROM wallet_ledger WHERE wallet_id = @wallet_id AND idempotency_key = 'DEMO-SEED-Y2-FX-05');
SET @wl_fx06 = (SELECT id FROM wallet_ledger WHERE wallet_id = @wallet_id AND idempotency_key = 'DEMO-SEED-Y2-FX-06');
...
VALUES
    ...
    (@wtc_id, 'EUR', 'IN', 'CARD_TOPUP',
     300.00, 0.00, 300.00,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-01', 'EUR 환전 충전', '2026-10-10 14:00:00'),
    -- CHF 충전 1: 200
    (@wtc_id, 'CHF', 'IN', 'CARD_TOPUP',
     200.00, 0.00, 200.00,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-02', 'CHF 환전 충전', '2026-12-20 14:00:00'),
    -- EUR 충전 2: 200
    (@wtc_id, 'EUR', 'IN', 'CARD_TOPUP',
     200.00, 300.00, 500.00,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-03', 'EUR 환전 충전', '2027-01-20 14:00:00'),
    -- CHF 충전 2: 300
    (@wtc_id, 'CHF', 'IN', 'CARD_TOPUP',
     300.00, 200.00, 500.00,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-04', 'CHF 환전 충전', '2027-02-20 14:00:00'),
    -- EUR 충전 3: 200
    (@wtc_id, 'EUR', 'IN', 'CARD_TOPUP',
     200.00, 500.00, 700.00,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-05', 'EUR 환전 충전', '2027-03-20 14:00:00'),
    -- CHF 충전 3: 400
    (@wtc_id, 'CHF', 'IN', 'CARD_TOPUP',
     400.00, 500.00, 900.00,
     'WALLET', @wallet_id, 'TRAVEL_CARD', @wtc_id,
     'DEMO-SEED-Y2-FX-06', 'CHF 환전 충전', '2027-03-20 14:30:00');

-- card ledger ID 참조
SET @cl_fx01 = (SELECT id FROM travel_card_ledger WHERE wallet_travel_card_id = @wtc_id AND idempotency_key = 'DEMO-SEED-Y2-FX-01');
SET @cl_fx02 = (SELECT id FROM travel_card_ledger WHERE wallet_travel_card_id = @wtc_id AND idempotency_key = 'DEMO-SEED-Y2-FX-02');
SET @cl_fx03 = (SELECT id FROM travel_card_ledger WHERE wallet_travel_card_id = @wtc_id AND idempotency_key = 'DEMO-SEED-Y2-FX-03');
SET @cl_fx04 = (SELECT id FROM travel_card_ledger WHERE wallet_travel_card_id = @wtc_id AND idempotency_key = 'DEMO-SEED-Y2-FX-04');
SET @cl_fx05 = (SELECT id FROM travel_card_ledger WHERE wallet_travel_card_id = @wtc_id AND idempotency_key = 'DEMO-SEED-Y2-FX-05');
SET @cl_fx06 = (SELECT id FROM travel_card_ledger WHERE wallet_travel_card_id = @wtc_id AND idempotency_key = 'DEMO-SEED-Y2-FX-06');
...
VALUES
    (@wallet_id, @wtc_id, 'EUR', 480000, 300.00, 'COMPLETED', 'DEMO-SEED-Y2-FX-01', 'EXT-FX-01', 1, '2026-10-10 14:00:00', '2026-10-10 14:00:00', @wl_fx01, @cl_fx01, 0),
    (@wallet_id, @wtc_id, 'CHF', 340000, 200.00, 'COMPLETED', 'DEMO-SEED-Y2-FX-02', 'EXT-FX-02', 1, '2026-12-20 14:00:00', '2026-12-20 14:00:00', @wl_fx02, @cl_fx02, 0),
    (@wallet_id, @wtc_id, 'EUR', 320000, 200.00, 'COMPLETED', 'DEMO-SEED-Y2-FX-03', 'EXT-FX-03', 1, '2027-01-20 14:00:00', '2027-01-20 14:00:00', @wl_fx03, @cl_fx03, 0),
    (@wallet_id, @wtc_id, 'CHF', 510000, 300.00, 'COMPLETED', 'DEMO-SEED-Y2-FX-04', 'EXT-FX-04', 1, '2027-02-20 14:00:00', '2027-02-20 14:00:00', @wl_fx04, @cl_fx04, 0),
    (@wallet_id, @wtc_id, 'EUR', 320000, 200.00, 'COMPLETED', 'DEMO-SEED-Y2-FX-05', 'EXT-FX-05', 1, '2027-03-20 14:00:00', '2027-03-20 14:00:00', @wl_fx05, @cl_fx05, 0),
    (@wallet_id, @wtc_id, 'CHF', 680000, 400.00, 'COMPLETED', 'DEMO-SEED-Y2-FX-06', 'EXT-FX-06', 1, '2027-03-20 14:30:00', '2027-03-20 14:30:00', @wl_fx06, @cl_fx06, 0);
...
VALUES
    -- France Day 1
    (@user_id, @trip_id, @country_france, 7, @eur_id, '2027-04-04 09:17:00', 'receipt_2_001.jpg', 'https://storage.tripass.com/demo/receipt_2_001.jpg', 'IMAGE', 'COMPLETED', 'Cafe de Flore', '카페 드 플로르', 18.50, 'Cafe de Flore\nCafe Creme x2\nCroissant x2\nTotal: 18.50 EUR'),
    (@user_id, @trip_id, @country_france, 5, @eur_id, '2027-04-04 10:34:00', 'receipt_2_002.jpg', 'https://storage.tripass.com/demo/receipt_2_002.jpg', 'IMAGE', 'COMPLETED', 'Musee du Louvre', '루브르 박물관', 17.00, 'Musee du Louvre\nBillet Adult x1\nTotal: 17.00 EUR'),
    (@user_id, @trip_id, @country_france, 1, @eur_id, '2027-04-04 11:51:00', 'receipt_2_003.jpg', 'https://storage.tripass.com/demo/receipt_2_003.jpg', 'IMAGE', 'COMPLETED', 'Le Petit Cler', '르 프티 클레르', 32.00, 'Le Petit Cler\nSteak Frites\nVin Rouge\nDessert\nTotal: 32.00 EUR'),
    (@user_id, @trip_id, @country_france, 5, @eur_id, '2027-04-04 12:08:00', 'receipt_2_004.jpg', 'https://storage.tripass.com/demo/receipt_2_004.jpg', 'IMAGE', 'COMPLETED', 'Tour Eiffel', '에펠탑', 26.80, 'Tour Eiffel\nSommet Adult x1\nTotal: 26.80 EUR'),
    (@user_id, @trip_id, @country_france, 2, @eur_id, '2027-04-04 13:25:00', 'receipt_2_005.jpg', 'https://storage.tripass.com/demo/receipt_2_005.jpg', 'IMAGE', 'COMPLETED', 'Metro RATP', '파리 메트로', 16.90, 'RATP\nCarnet 10 Tickets\nTotal: 16.90 EUR'),
    -- France Day 2
    (@user_id, @trip_id, @country_france, 1, @eur_id, '2027-04-05 14:42:00', 'receipt_2_006.jpg', 'https://storage.tripass.com/demo/receipt_2_006.jpg', 'IMAGE', 'COMPLETED', 'Boulangerie Montmartre', '몽마르뜨 빵집', 8.50, 'Boulangerie\nPain au Chocolat x2\nCafe x1\nTotal: 8.50 EUR'),
    (@user_id, @trip_id, @country_france, 5, @eur_id, '2027-04-05 15:59:00', 'receipt_2_007.jpg', 'https://storage.tripass.com/demo/receipt_2_007.jpg', 'IMAGE', 'COMPLETED', 'Musee d''Orsay', '오르세 미술관', 16.00, 'Musee d''Orsay\nBillet Adult x1\nTotal: 16.00 EUR'),
    (@user_id, @trip_id, @country_france, 1, @eur_id, '2027-04-05 16:16:00', 'receipt_2_008.jpg', 'https://storage.tripass.com/demo/receipt_2_008.jpg', 'IMAGE', 'COMPLETED', 'Les Deux Magots', '레 되 마고', 28.00, 'Les Deux Magots\nSoupe a l''oignon\nConfit de Canard\nTotal: 28.00 EUR'),
    (@user_id, @trip_id, @country_france, 4, @eur_id, '2027-04-05 17:33:00', 'receipt_2_009.jpg', 'https://storage.tripass.com/demo/receipt_2_009.jpg', 'IMAGE', 'COMPLETED', 'Shakespeare and Company', '셰익스피어 앤 컴퍼니', 22.00, 'Shakespeare and Company\nBooks x2\nTotal: 22.00 EUR'),
    -- France Day 3
    (@user_id, @trip_id, @country_france, 5, @eur_id, '2027-04-06 18:50:00', 'receipt_2_010.jpg', 'https://storage.tripass.com/demo/receipt_2_010.jpg', 'IMAGE', 'COMPLETED', 'Chateau de Versailles', '베르사유 궁전', 21.00, 'Chateau de Versailles\nBillet Adult x1\nTotal: 21.00 EUR'),
    (@user_id, @trip_id, @country_france, 7, @eur_id, '2027-04-06 19:07:00', 'receipt_2_011.jpg', 'https://storage.tripass.com/demo/receipt_2_011.jpg', 'IMAGE', 'COMPLETED', 'Angelina Versailles', '앙젤리나 베르사유', 15.50, 'Angelina\nChocolat Chaud\nMont Blanc\nTotal: 15.50 EUR'),
    (@user_id, @trip_id, @country_france, 5, @eur_id, '2027-04-06 08:24:00', 'receipt_2_012.jpg', 'https://storage.tripass.com/demo/receipt_2_012.jpg', 'IMAGE', 'COMPLETED', 'Centre Pompidou', '퐁피두 센터', 15.00, 'Centre Pompidou\nBillet Adult x1\nTotal: 15.00 EUR'),
    (@user_id, @trip_id, @country_france, 1, @eur_id, '2027-04-06 09:41:00', 'receipt_2_013.jpg', 'https://storage.tripass.com/demo/receipt_2_013.jpg', 'IMAGE', 'COMPLETED', 'Le Marais Falafel', '마레 팔라펠', 9.50, 'L''As du Fallafel\nFalafel Special\nTotal: 9.50 EUR'),
    -- France Day 4
    (@user_id, @trip_id, @country_france, 5, @eur_id, '2027-04-07 10:58:00', 'receipt_2_014.jpg', 'https://storage.tripass.com/demo/receipt_2_014.jpg', 'IMAGE', 'COMPLETED', 'Musee de l''Orangerie', '오랑주리 미술관', 12.50, 'Musee de l''Orangerie\nBillet Adult x1\nTotal: 12.50 EUR'),
    (@user_id, @trip_id, @country_france, 4, @eur_id, '2027-04-07 11:15:00', 'receipt_2_015.jpg', 'https://storage.tripass.com/demo/receipt_2_015.jpg', 'IMAGE', 'COMPLETED', 'Galeries Lafayette', '갤러리 라파예트', 85.00, 'Galeries Lafayette\nParfum\nTotal: 85.00 EUR'),
    (@user_id, @trip_id, @country_france, 5, @eur_id, '2027-04-07 12:32:00', 'receipt_2_016.jpg', 'https://storage.tripass.com/demo/receipt_2_016.jpg', 'IMAGE', 'COMPLETED', 'Palais Garnier', '오페라 가르니에', 14.00, 'Palais Garnier\nVisite libre\nTotal: 14.00 EUR'),
    (@user_id, @trip_id, @country_france, 5, @eur_id, '2027-04-07 13:49:00', 'receipt_2_017.jpg', 'https://storage.tripass.com/demo/receipt_2_017.jpg', 'IMAGE', 'COMPLETED', 'Bateaux Mouches', '바토 무슈', 15.00, 'Bateaux Mouches\nCroisiere 1h\nTotal: 15.00 EUR'),
    (@user_id, @trip_id, @country_france, 1, @eur_id, '2027-04-07 14:06:00', 'receipt_2_018.jpg', 'https://storage.tripass.com/demo/receipt_2_018.jpg', 'IMAGE', 'COMPLETED', 'Le Bouillon Chartier', '르 부이용 샤르티에', 24.00, 'Le Bouillon Chartier\nEscargots\nBoeuf Bourguignon\nTotal: 24.00 EUR'),
    -- France Day 5
    (@user_id, @trip_id, @country_france, 7, @eur_id, '2027-04-08 15:23:00', 'receipt_2_019.jpg', 'https://storage.tripass.com/demo/receipt_2_019.jpg', 'IMAGE', 'COMPLETED', 'Canal Saint-Martin Cafe', '생마르탱 카페', 12.00, 'Cafe Craft\nFlat White x2\nTotal: 12.00 EUR'),
    (@user_id, @trip_id, @country_france, 5, @eur_id, '2027-04-08 16:40:00', 'receipt_2_020.jpg', 'https://storage.tripass.com/demo/receipt_2_020.jpg', 'IMAGE', 'COMPLETED', 'Bateaux Mouches Sunset', '바토 무슈 석양', 15.00, 'Bateaux Mouches\nCroisiere Sunset\nTotal: 15.00 EUR'),
    (@user_id, @trip_id, @country_france, 1, @eur_id, '2027-04-08 17:57:00', 'receipt_2_021.jpg', 'https://storage.tripass.com/demo/receipt_2_021.jpg', 'IMAGE', 'COMPLETED', 'Chez Janou', '셰 자누', 38.00, 'Chez Janou\nMousse au Chocolat\nPlat du Jour\nVin\nTotal: 38.00 EUR'),
    -- France Day 6 (travel day)
    (@user_id, @trip_id, @country_france, 1, @eur_id, '2027-04-09 18:14:00', 'receipt_2_022.jpg', 'https://storage.tripass.com/demo/receipt_2_022.jpg', 'IMAGE', 'COMPLETED', 'Gare de Lyon Relay', '리옹역 릴레이', 9.80, 'Relay\nSandwich\nEau minerale\nTotal: 9.80 EUR'),
    -- Switzerland Day 1 (4/9)
    (@user_id, @trip_id, @country_swiss, 1, @chf_id, '2027-04-09 19:31:00', 'receipt_2_023.jpg', 'https://storage.tripass.com/demo/receipt_2_023.jpg', 'IMAGE', 'COMPLETED', 'Zeughauskeller', '초이크하우스켈러', 42.00, 'Zeughauskeller\nZurcher Geschnetzeltes\nBier\nTotal: CHF 42.00'),
    (@user_id, @trip_id, @country_swiss, 4, @chf_id, '2027-04-09 08:48:00', 'receipt_2_024.jpg', 'https://storage.tripass.com/demo/receipt_2_024.jpg', 'IMAGE', 'COMPLETED', 'Sprungli', '슈프륑리', 18.50, 'Confiserie Sprungli\nLuxemburgerli Box\nTotal: CHF 18.50'),
    -- Switzerland Day 2 (4/10)
    (@user_id, @trip_id, @country_swiss, 5, @chf_id, '2027-04-10 09:05:00', 'receipt_2_025.jpg', 'https://storage.tripass.com/demo/receipt_2_025.jpg', 'IMAGE', 'COMPLETED', 'Jungfrau Railways', '융프라우 철도', 230.00, 'Jungfrau Railways\nTop of Europe Ticket\nTotal: CHF 230.00'),
    (@user_id, @trip_id, @country_swiss, 1, @chf_id, '2027-04-10 10:22:00', 'receipt_2_026.jpg', 'https://storage.tripass.com/demo/receipt_2_026.jpg', 'IMAGE', 'COMPLETED', 'Restaurant Eigerblick', '아이거블릭 레스토랑', 35.00, 'Restaurant Eigerblick\nRosti mit Spiegelei\nKaffee\nTotal: CHF 35.00'),
    (@user_id, @trip_id, @country_swiss, 4, @chf_id, '2027-04-10 11:39:00', 'receipt_2_027.jpg', 'https://storage.tripass.com/demo/receipt_2_027.jpg', 'IMAGE', 'COMPLETED', 'Coop Interlaken', '쿱 인터라켄', 15.80, 'Coop\nWasser\nSchokolade\nSnacks\nTotal: CHF 15.80'),
    -- Switzerland Day 3 (4/11)
    (@user_id, @trip_id, @country_swiss, 5, @chf_id, '2027-04-11 12:56:00', 'receipt_2_028.jpg', 'https://storage.tripass.com/demo/receipt_2_028.jpg', 'IMAGE', 'COMPLETED', 'Pilatus Bahnen', '필라투스 철도', 72.00, 'Pilatus Bahnen AG\nGolden Round Trip\nTotal: CHF 72.00'),
    (@user_id, @trip_id, @country_swiss, 5, @chf_id, '2027-04-11 13:13:00', 'receipt_2_029.jpg', 'https://storage.tripass.com/demo/receipt_2_029.jpg', 'IMAGE', 'COMPLETED', 'SGV Luzern', '루체른 유람선', 45.00, 'SGV\nSchifffahrt Luzern\nTotal: CHF 45.00'),
    (@user_id, @trip_id, @country_swiss, 1, @chf_id, '2027-04-11 14:30:00', 'receipt_2_030.jpg', 'https://storage.tripass.com/demo/receipt_2_030.jpg', 'IMAGE', 'COMPLETED', 'Wirtshaus Galliker', '갈리커 레스토랑', 38.00, 'Wirtshaus Galliker\nKalbsgeschnetzeltes\nSalat\nTotal: CHF 38.00'),
    (@user_id, @trip_id, @country_swiss, 4, @chf_id, '2027-04-11 15:47:00', 'receipt_2_031.jpg', 'https://storage.tripass.com/demo/receipt_2_031.jpg', 'IMAGE', 'COMPLETED', 'Bachmann Confiserie', '바흐만 과자점', 12.50, 'Bachmann\nLuzerner Lebkuchen\nTotal: CHF 12.50'),
    -- Switzerland Day 4 (4/12)
    (@user_id, @trip_id, @country_swiss, 5, @chf_id, '2027-04-12 16:04:00', 'receipt_2_032.jpg', 'https://storage.tripass.com/demo/receipt_2_032.jpg', 'IMAGE', 'COMPLETED', 'Skywings Paragliding', '스카이윙스 패러글라이딩', 180.00, 'Skywings Interlaken\nTandem Paragliding\nTotal: CHF 180.00'),
    (@user_id, @trip_id, @country_swiss, 5, @chf_id, '2027-04-12 17:21:00', 'receipt_2_033.jpg', 'https://storage.tripass.com/demo/receipt_2_033.jpg', 'IMAGE', 'COMPLETED', 'Harder Kulm Bahn', '하더 쿨름 케이블카', 32.00, 'Harder Kulm\nFunicular Return\nTotal: CHF 32.00'),
    (@user_id, @trip_id, @country_swiss, 1, @chf_id, '2027-04-12 18:38:00', 'receipt_2_034.jpg', 'https://storage.tripass.com/demo/receipt_2_034.jpg', 'IMAGE', 'COMPLETED', 'Restaurant Baren', '베렌 레스토랑', 45.00, 'Restaurant Baren\nFondue Moitie-Moitie\nBier\nTotal: CHF 45.00'),
    -- Portugal Day 1 (4/14)
    (@user_id, @trip_id, @country_portugal, 1, @eur_id, '2027-04-14 19:55:00', 'receipt_2_035.jpg', 'https://storage.tripass.com/demo/receipt_2_035.jpg', 'IMAGE', 'COMPLETED', 'Pasteis de Belem', '파스테이스 드 벨렘', 8.50, 'Pasteis de Belem\nPastel de Nata x4\nCafe x2\nTotal: 8.50 EUR'),
    (@user_id, @trip_id, @country_portugal, 5, @eur_id, '2027-04-14 08:12:00', 'receipt_2_036.jpg', 'https://storage.tripass.com/demo/receipt_2_036.jpg', 'IMAGE', 'COMPLETED', 'Torre de Belem', '벨렘탑', 8.00, 'Torre de Belem\nBilhete Adulto\nTotal: 8.00 EUR'),
    (@user_id, @trip_id, @country_portugal, 1, @eur_id, '2027-04-14 09:29:00', 'receipt_2_037.jpg', 'https://storage.tripass.com/demo/receipt_2_037.jpg', 'IMAGE', 'COMPLETED', 'Cervejaria Ramiro', '세르베자리아 하미루', 35.00, 'Cervejaria Ramiro\nGambas al Ajillo\nPrego\nVinho Verde\nTotal: 35.00 EUR'),
    -- Portugal Day 2 (4/15)
    (@user_id, @trip_id, @country_portugal, 5, @eur_id, '2027-04-15 10:46:00', 'receipt_2_038.jpg', 'https://storage.tripass.com/demo/receipt_2_038.jpg', 'IMAGE', 'COMPLETED', 'Castelo de Sao Jorge', '상 조르제 성', 10.00, 'Castelo de Sao Jorge\nBilhete\nTotal: 10.00 EUR'),
    (@user_id, @trip_id, @country_portugal, 1, @eur_id, '2027-04-15 11:03:00', 'receipt_2_039.jpg', 'https://storage.tripass.com/demo/receipt_2_039.jpg', 'IMAGE', 'COMPLETED', 'Time Out Market', '타임아웃 마켓', 22.00, 'Time Out Market\nBacalhau a Bras\nGinjinha\nTotal: 22.00 EUR'),
    (@user_id, @trip_id, @country_portugal, 2, @eur_id, '2027-04-15 12:20:00', 'receipt_2_040.jpg', 'https://storage.tripass.com/demo/receipt_2_040.jpg', 'IMAGE', 'COMPLETED', 'Carris Lisboa', '리스본 교통', 7.00, 'Carris\nViva Viagem 24h\nTotal: 7.00 EUR'),
    -- Portugal Day 3 (4/16)
    (@user_id, @trip_id, @country_portugal, 5, @eur_id, '2027-04-16 13:37:00', 'receipt_2_041.jpg', 'https://storage.tripass.com/demo/receipt_2_041.jpg', 'IMAGE', 'COMPLETED', 'Palacio da Pena', '페나 궁전', 14.00, 'Palacio Nacional da Pena\nBilhete\nTotal: 14.00 EUR'),
    (@user_id, @trip_id, @country_portugal, 5, @eur_id, '2027-04-16 14:54:00', 'receipt_2_042.jpg', 'https://storage.tripass.com/demo/receipt_2_042.jpg', 'IMAGE', 'COMPLETED', 'Quinta da Regaleira', '헤갈레이라 별장', 10.00, 'Quinta da Regaleira\nEntrada\nTotal: 10.00 EUR'),
    (@user_id, @trip_id, @country_portugal, 1, @eur_id, '2027-04-16 15:11:00', 'receipt_2_043.jpg', 'https://storage.tripass.com/demo/receipt_2_043.jpg', 'IMAGE', 'COMPLETED', 'Restaurante Regional', '지역 레스토랑', 18.00, 'Restaurante Regional\nFrango Piri-Piri\nArroz\nTotal: 18.00 EUR'),
    -- Portugal Day 4 (4/17)
    (@user_id, @trip_id, @country_portugal, 2, @eur_id, '2027-04-17 16:28:00', 'receipt_2_044.jpg', 'https://storage.tripass.com/demo/receipt_2_044.jpg', 'IMAGE', 'COMPLETED', 'CP Comboios', '포르투갈 기차', 25.00, 'CP\nLisboa-Porto AP\nTotal: 25.00 EUR'),
    (@user_id, @trip_id, @country_portugal, 5, @eur_id, '2027-04-17 17:45:00', 'receipt_2_045.jpg', 'https://storage.tripass.com/demo/receipt_2_045.jpg', 'IMAGE', 'COMPLETED', 'Livraria Lello', '렐루 서점', 5.00, 'Livraria Lello\nVoucher Entrada\nTotal: 5.00 EUR'),
    (@user_id, @trip_id, @country_portugal, 1, @eur_id, '2027-04-17 18:02:00', 'receipt_2_046.jpg', 'https://storage.tripass.com/demo/receipt_2_046.jpg', 'IMAGE', 'COMPLETED', 'Cafe Majestic', '카페 마제스틱', 16.00, 'Cafe Majestic\nFrancesinha\nCerveja\nTotal: 16.00 EUR'),
    -- Portugal Day 5 (4/18)
    (@user_id, @trip_id, @country_portugal, 4, @eur_id, '2027-04-18 19:19:00', 'receipt_2_047.jpg', 'https://storage.tripass.com/demo/receipt_2_047.jpg', 'IMAGE', 'COMPLETED', 'LX Factory Market', 'LX 팩토리', 28.00, 'LX Factory\nAzulejo Art\nSouvenir\nTotal: 28.00 EUR'),
    (@user_id, @trip_id, @country_portugal, 5, @eur_id, '2027-04-18 08:36:00', 'receipt_2_048.jpg', 'https://storage.tripass.com/demo/receipt_2_048.jpg', 'IMAGE', 'COMPLETED', 'Cristo Rei', '크리스투 레이', 6.00, 'Santuario Cristo Rei\nElevador\nTotal: 6.00 EUR');

-- ============================================================================
-- 27. 영수증 품목 (주요 영수증에 2~4개씩)
-- ============================================================================
-- receipt_2_001 (Cafe de Flore)
SET @r001 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_001.jpg');
SET @r003 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_003.jpg');
SET @r008 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_008.jpg');
SET @r015 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_015.jpg');
SET @r018 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_018.jpg');
SET @r021 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_021.jpg');
SET @r026 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_026.jpg');
SET @r030 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_030.jpg');
SET @r034 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_034.jpg');
SET @r037 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_037.jpg');
SET @r039 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_039.jpg');
SET @r046 = (SELECT id FROM receipts WHERE user_id = @user_id AND file_name = 'receipt_2_046.jpg');
