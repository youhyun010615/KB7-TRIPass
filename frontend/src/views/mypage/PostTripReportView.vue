    ...
    <template v-else>
      <div ref="reportContent" class="pdf-content">
        ...
        <section class="card overview">
          <div class="overview-title"><h2>여행 지출 한눈에 보기</h2><em>{{ r.remaining >= 0 ? '예산 안에서 여행을 마쳤어요' : '예산을 초과했어요' }}</em></div>
          <div class="overview-head"><div><span>총</span><strong>{{ money(r.spent) }}</strong><span>지출</span></div></div>
...
.overview-title{display:flex;align-items:center;justify-content:space-between;gap:8px}.overview-title em{flex:0 0 auto;padding:7px 9px;border-radius:999px;background:#e8f8f3;color:#087f61;font-size:8px;font-style:normal;font-weight:850;line-height:1;white-space:nowrap}.report-modal .overview-title em{font-size:10px;line-height:1.25}
</style>
