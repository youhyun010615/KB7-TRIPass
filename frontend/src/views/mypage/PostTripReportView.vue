    ...
    <template v-else>
      <div ref="reportContent" class="pdf-content">
        ...
        <section class="card next-trip">
          <h2>다음 여행 준비 제안</h2>
          <p v-if="highestCountryUsage && highestCountryUsage.amount > 0"><i>↗</i><span><b>{{ highestCountryUsage.name }}</b>에서 예산의 {{ highestCountryUsage.usage }}%를 사용했어요. 다음 여행에서는 이 국가의 여유 예산을 더 확보해 보세요.</span></p>
          <p v-if="topCategory"><i>★</i><span>가장 큰 <b>{{ topCategory.name }}</b> 예산 {{ money(topCategory.amount) }}을 다음 여행 계획에서 먼저 확보해 보세요.</span></p>
          <p v-if="!topCategory && (!highestCountryUsage || highestCountryUsage.amount <= 0)" class="empty-state">지출 내역이 쌓이면 이번 여행을 바탕으로 다음 여행 준비 방법을 제안해 드려요.</p>
        </section>
      </div>
