...
const tripId=computed(()=>Number(route.query.tripId||1)); const report=computed(()=>store.getReport(tripId.value))
</script>

<template><main class="page"><header><button @click="router.back()">‹</button><h1>여행 리포트</h1><span/></header>
  <section class="ticket"><small>TRIP REPORT ARCHIVE</small><div><h2>{{ report.trip.flags }} {{ report.trip.title }}</h2><b>D-{{ report.trip.dDay }}</b></div><i/><p>생성된 리포트</p><div class="total"><strong>2개</strong><em>여행 전 · 여행 후</em></div></section>
  <h3>리포트 목록</h3>
