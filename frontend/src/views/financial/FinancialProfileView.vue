        ...
        <div class="connection-content">
          ...
          <section class="linked-card-list">
            <article v-for="card in linkedCards" :key="card.id ?? card.maskedCardNumber" class="linked-card-item">
              <span class="linked-card-icon">
                <img
                  v-if="cardCompanyLogo(selectedCardInstitution)"
                  :src="cardCompanyLogo(selectedCardInstitution)"
                  :alt="`${selectedCardInstitution?.institutionName || '카드사'} 로고`"
                >
                <span v-else aria-hidden="true">▰</span>
              </span><div><strong>{{ card.cardName }}</strong><small>{{ cardTypeName(card.cardType) }} · {{ card.maskedCardNumber || '카드번호 비공개' }}</small></div>
            </article>
          </section>
        </div>
