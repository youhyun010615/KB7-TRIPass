...
const todaySchedules = computed(() => store.sortedSchedules.filter(item => item.date === store.today))
...
const upcomingGroups = computed(() => {
  ...
  store.sortedSchedules.filter(item => item.date > store.today).forEach(item => {
