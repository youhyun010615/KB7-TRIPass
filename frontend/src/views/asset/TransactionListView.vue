...
onMounted(async () => {
  try {
    ...
    accountMap.value = Object.fromEntries(
      accounts.map((a) => [a.id, a.accountName]),
    );
    cardMap.value = Object.fromEntries(
      cards.map((card) => [card.id, card.cardName]),
    );
...
const groups = computed(() => {
  ...
  realTransactions.value.forEach((t) => {
    const [y, mo, d] = Array.isArray(t.transactionDate)
      ? t.transactionDate
      : t.transactionDate.split('-').map(Number);
    const date = `${y}-${String(mo).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
    if (date < startDate.value || date > endDate.value) return;
    const amount =
      t.transactionType === 'DEPOSIT' ? Number(t.amount) : -Number(t.amount);
    ...
    const [h = 0, m = 0] = Array.isArray(t.transactionTime)
      ? t.transactionTime
      : (t.transactionTime ?? '00:00').split(':').map(Number);
    allItems.push({
      id: t.id,
      date,
      dateLabel: label,
      ...
      method: t.cardId
        ? (cardMap.value[t.cardId] ?? '연동 카드')
        : (accountMap.value[t.accountId] ?? '연동 계좌'),
      amount,
      time: `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}`,
      balanceAfter: t.cardId ? null : Number(t.balanceAfter ?? 0),
      memo: t.memo ?? '',
  ...
  allItems.sort(
    (a, b) =>
      b.date.localeCompare(a.date) || b.time?.localeCompare(a.time ?? '') || 0,
  );
  ...
  <main class="list-page">
    <header>
      ...
      <button
        ...
        aria-label="거래내역 캘린더"
        class="cal-btn"
        @click="router.push('/asset/transactions/calendar')"
      >
        <svg
          width="21"
          height="21"
    ...
    <section class="summary">
      <small>통장 {{ realAccountCount }}개 · 카드 {{ realCardCount }}장</small
      ><b>총 {{ totalCount }}건의 거래내역</b>
    </section>
    <section class="date-filter">
      <label
        ><span>시작일</span
        ><input v-model="startDate" type="date" :max="endDate" /></label
      ><i>~</i
      ><label
        ><span>종료일</span
    ...
    <TransactionGroups
      ...
      @select="
        router.push({
          path: `/asset/transactions/${$event.id}`,
          state: { item: $event },
        })
      "
    />
...
.summary {
  padding: 15px;
  border-radius: 14px;
  background: linear-gradient(135deg, #173f8d, #2866bd);
  color: #fff;
}
.summary small,
.summary b {
