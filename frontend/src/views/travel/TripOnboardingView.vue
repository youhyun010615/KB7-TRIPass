...
const completedCount = computed(() => Number(hasTrip.value) + Number(hasLinkedAccount.value))
  ...
  <main class="onboarding-page">
    ...
    <template v-else>
      <header class="setup-hero">
        ...
        <p class="hero-copy">여행 계획과 계좌를 등록하면<br>맞춤 저축 여정이 시작돼요.</p>
      </header>
      ...
      <section class="setup-content">
        <article class="setup-ticket">
          ...
          <div class="ticket-band">
            ...
            <b>{{ completedCount }}/2</b>
          </div>

          <div class="progress-copy">
            <div>
              <small>여행 준비 진행률</small>
              <strong>{{ completedCount === 2 ? '준비가 완료됐어요' : `${2 - completedCount}단계가 남았어요` }}</strong>
            </div>
            <em>{{ completedCount * 50 }}%</em>
          </div>
          <div class="progress-track"><i :style="{ width: `${completedCount * 50}%` }" /></div>
          ...
          <p class="security-copy">계좌 인증 정보는 연결 과정에서만 안전하게 사용돼요.</p>
        </article>

        <button type="button" class="start-button" @click="finishOnboarding">{{ completedCount === 2 ? 'TRIPass 시작하기' : '일단 둘러볼게요' }}</button>
        <button v-if="completedCount < 2" type="button" class="skip-button" @click="finishOnboarding">SKIP 하기</button>
      </section>
    </template>
  </main>
</template>
