...
<template>
  <main class="prepaid-page">
    <header><button @click="router.back()">‹</button><div><h1>사전 지불 금액 등록</h1><p>여행을 위해 미리 지불한 항목 정보를 입력해 주세요.</p></div><span aria-hidden="true"></span></header>
    <section class="field" :class="{ error: touched && !form.name.trim() }"><div class="label"><span>▦</span><b>항목명</b><small>알아보기 쉬운 이름으로 수정할 수 있어요.</small></div><input v-model="form.name" maxlength="40" placeholder="항목명을 입력해 주세요"><div class="original-name"><span>원 거래 항목명</span><b>{{ transaction?.merchant || '-' }}</b><em>수정 불가</em></div></section>
...
<style scoped>
.prepaid-page{width:min(100%,390px);min-height:100vh;margin:0 auto;padding:14px 14px 24px;background:#f7f4ee;color:#151f33}header{display:grid;grid-template-columns:36px 1fr 36px;align-items:start;margin-bottom:20px}header button{display:grid;width:36px;height:36px;place-items:center;border-radius:12px;background:#fff;color:#193d82;font-size:24px;font-weight:700;box-shadow:0 5px 16px rgba(36,72,117,.07)}h1{font-size:19px;font-weight:900;text-align:center}header p{margin-top:6px;color:#94a3b8;font-size:9px;text-align:center}.field{position:relative;margin-bottom:10px;padding:14px;border:1px solid #dfe6f1;border-radius:16px;background:white;box-shadow:0 4px 10px #1e34620b}.field:focus-within{border-color:#86b7ff}.field.error{border-color:#f29a9a;background:#fffafa}.field.auto{background:#f8fbff}.label{display:grid;grid-template-columns:25px 1fr auto;align-items:center;margin-bottom:12px}.label>span{color:#3181ff}.label b{font-size:12px}.label i{margin-left:8px;color:#ef4444;font-size:10px}.label small{color:#64748b;font-size:8px}.field input,.field select,.field textarea,.money-input{width:100%;border:1px solid #d9e2f0;border-radius:11px;background:white;color:#334155;font-size:11px;outline:none}.field>input,.field select{height:48px;padding:0 13px}.field input:read-only{color:#536174;background:#f8fafc}.original-name{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:8px;margin-top:8px;padding:10px 12px;border-radius:9px;background:#f3f5f8}.original-name span{color:#8793a4;font-size:8px}.original-name b{overflow:hidden;color:#536174;font-size:10px;text-overflow:ellipsis;white-space:nowrap}.original-name em{padding:3px 6px;border-radius:8px;background:#e5e9ef;color:#8a95a4;font-size:7px;font-style:normal}.field textarea{height:78px;padding:13px;resize:none}.money-input{display:flex;align-items:center;padding:0 12px}.money-input input{height:46px;border:0}.money-input b{font-size:11px}.amount{border-color:#cdeedd;background:#fbfffd}.count{position:absolute;right:24px;bottom:23px;color:#94a3b8;font-size:8px}.error-copy{margin:4px 4px 10px;color:#e5484d;font-size:10px}.cta{width:100%;height:54px;margin-top:18px;border-radius:12px;background:#174494;color:white;font-size:14px;font-weight:900}.cta:disabled{background:#aab5c7}
</style>
